...
const ProductDescriptionInputSchema = z.object({
  ...
  establishmentName: z.string().optional().describe('O nome da clínica.'),
  nicheContext: z.string().optional().describe('Contexto da clínica (ex: clássica, moderna, urbana).'),
});
...
const prompt = ai.definePrompt({
  ...
  Sua tarefa é criar uma descrição de produto IRRESISTÍVEL para a clínica "{{{establishmentName}}}".
  
  Contexto da Clínica: {{{nicheContext}}}
  ...
  4. Use gatilhos mentais como exclusividade, autoridade da clínica e transformação visual.
